      ...
      <div className="relative max-w-4xl px-10 py-6 mx-auto bg-white rounded-lg shadow-md text-left overflow-hidden">
        ...
        <div className="mt-2">
          <Link passHref href={`/blog/${blog?.slug}`}>
            <p className="cursor-pointer text-2xl font-bold text-gray-700 hover:underline md:w-4/6">
              {blog?.title}
            </p>
          </Link>
          ...
          <div>
            <Link passHref href={`/author/${blog?.author?.user?.username}`}>
              <div className="cursor-pointer flex items-center">
                <div className="hidden object-cover w-10 h-10 mx-4 rounded-full overflow-hidden sm:block bg-blue-100">
                  <Image
                    src={
                      blog?.author?.picture
                        ? blog?.author?.picture
